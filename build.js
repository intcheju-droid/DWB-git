...
function indexPage(articles){
  ...
  if(briefs.length){
    var bh = briefs[0];
    briefBox = '<div class="box" id="briefing"><h3>오늘의 브리핑</h3>'
      + '<p style="margin:.2rem 0 .1rem"><a href="articles/' + bh.slug + '.html"><strong>' + esc(bh.meta.title) + '</strong></a></p>'
      + '<p style="font-size:.82em;color:var(--sub);margin:0 0 .4rem">' + fmtDate(bh.meta.date) + ' · ' + esc(bh.meta.author || '') + '</p>'
      + (briefs.length > 1
          ? '<ul style="list-style:none;margin:0;padding:0;border-top:1px solid var(--line)">'
            + briefs.slice(1, 6).map(function(a){
                return '<li style="padding:.3rem 0;font-size:.88em"><a href="articles/' + a.slug + '.html">' + fmtDate(a.meta.date) + ' 브리핑</a></li>';
              }).join('')
            + '</ul>'
          : '')
